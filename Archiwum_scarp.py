...
for page in range(1, 160):
    response = requests.get(
        'https://archiwum.allegro.pl/kategoria/obraz-i-grafika-karty-graficzne-257236?p={}'.format(page))
    ...
    for post in posts:
        ...
        if name is not None:
            ...
            price = re.search('(\d+ ?\d*|Nie było ofert kupna)', price).group(1)
            price_offered = re.search('fee8042">(\d\d? ?\d*)', str(post)).group(1)
            re.search('(\d\d? ?\d*)',str('1 700')).group(1)
            df['Nazwa'] = name.replace("Kup podobny do ", "")
            df['Cena kupiona'] = price.replace(" ", "")
            df['Cena zaoferowana'] = price_offered.replace(" ", "")
            df['Link'] = link
...
global_df['Link'][93]
global_df.to_csv('pierwsze_archiwum.csv')
...
global_df['Cena'] = 0
global_df['Cena'] = global_df['Cena zaoferowana']

global_df = pd.read_csv('archiwum_1.csv')
#global_df.rename(columns={"Stan": "Stan:"}, inplace=True)
